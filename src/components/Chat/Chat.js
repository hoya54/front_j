...
const Chat = () => {
  ...
  async function enter_chat_room(room_id, chatTarget) {
    ...
    stompClient.current.subscribe(
      ...
      (message) => {
        ...
        console.log(msg);
        [msg.year, msg.month, msg.day] = msg.createdDate
          .split("T")[0]
          .split("-");
  ...
  function getRoomList() {
    ...
    dispatch(getChatRoomList(id, role)).then((data) => {
      setRoomList(data);
  ...
  async function getChatList(room_id) {
    axios.get("/api/chat/load/" + room_id).then((res) => {
      res.data.map((Adata) => {
        if (Adata.writer === name) Adata.me = true;
        else Adata.me = false;
  ...
  function showChatList() {
    if (chatlist) {
      return chatlist.map((chat, index) => {
        if (
          index &&
          chat.day === chatlist[index - 1].day &&
          chat.month === chatlist[index - 1].month &&
          chat.year === chatlist[index - 1].year
        ) {
          return (
            <div key={index}>
              <ChatList {...chat} />
        ...
        return (
          <div key={index}>
            <div className={styles.chat_list_day}>
              {chat.year}년 {chat.month}월 {chat.day}일
            </div>
            <ChatList {...chat} />
          </div>
        );
          ...
          <div className={styles.chat_title_box}>
            {chatRoom
              ? [
                  <div
                    key="back"
                    className={styles.chat_exit_btn}
                    onClick={() => {
                      leave_chat_room();
                    }}
                  >
                  ...
                  <div key="room_name">{chatTarget} 님</div>,
                ]
              : [
                  <div key="whitespace">&nbsp;&nbsp;</div>,
                  <div key="roomlist">채팅방</div>,
                ]}
            <div
              className={styles.chat_exit_btn}
              onClick={() => {
