...
export const reviewList = (pagenum) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `/api/business/opinion/review/list/${pagenum}`
    );
    console.log(response);
...
export const trainerReview = (id, page) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `/api/business/opinion/review/trainer/list/${id}/${page}`
    );
    dispatch(etcActions.dataSuccess());
    return response.data;
  } catch (error) {
...
export const clientReview = (id, page) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `/api/business/opinion/review/user/list/${id}/${page}`
    );
    dispatch(etcActions.dataSuccess());
    return response.data;
  } catch (error) {
...
export const clientSchedule = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `/api/business/reservation/user/list/${id}`
    );
    console.log(response.data);
    dispatch(etcActions.dataSuccess());
    return response.data;
...
export const getChatRoom =
  (myId, myRole, myName, targetId, targetName) => async (dispatch) => {
    dispatch(etcActions.dataRequest());
    try {
      const response = await axios.get(
        "/api/chat/channel/" +
...
export const nameSearch = (page, word) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `/api/trainer/search/name/${page}/${word}`
    );
...
export const dateSearch = (page, date) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `/api/trainer/search/date/${page}/${date}`
    );
...
export const getDaySchedule = (id, day, page) => async (dispatch) => {
  ...
  try {
    ...
    return await axios
      .get(
        `/api/business/reservation/page/${id}/${day.getFullYear()}/${
          day.getMonth() + 1
        }/${day.getDate()}/${page - 1}`
      )
      .then((res) => res.data);
...
export const getMyClient = (id, page) => async (dispatch) => {
  ...
  try {
    dispatch(etcActions.dataSuccess());
    return await axios
      .post(`/api/user/userList/${page - 1}`, { id: id })
      .then((res) => res.data);
  } catch (err) {
    dispatch(etcActions.dataFailure());
...
export const getTodayLesson = (id) => async (dispatch) => {
  ...
  try {
    dispatch(etcActions.dataSuccess());
    return await axios
      .get(`/api/business/reservation/trainer/reserved/list/${id}`)
      .then((res) => {
...
export const writeReview =
  (userId, trainerId, star, memo, userName, trainerName) =>
  async (dispatch) => {
    dispatch(etcActions.dataRequest());
    try {
      dispatch(etcActions.dataSuccess());
      return await axios
        .post("/api/business/opinion/review/write", {
          writerId: userId,
          targetId: trainerId,
          star: star,
          memo: memo,
...
export const changeTrainerInfo = (email, phone) => async (dispatch) => {
  try {
    return await axios
      .post("/api/trainer/info/update/" + email, { phone })
      .then((res) => res.data);
  } catch (err) {
